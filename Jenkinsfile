...
# --- added: install helm (needed for AWS LB Controller) ---
...
eksctl utils associate-iam-oidc-provider \
  --cluster "$CLUSTER" --region "$REGION" --approve || true

# IAM role for controller (create if missing)
if ! aws iam get-role --role-name "$ROLE_NAME" >/dev/null 2>&1 ; then
  eksctl create iamserviceaccount \
    --cluster "$CLUSTER" --name "$SA_NAME" --namespace "$SA_NS" \
    --role-name "$ROLE_NAME" --attach-policy-arn "$POLICY_ARN" \
...
'''
    ...
    /*--------------------------------------------------------*/
    stage('Deploy to EKS (Fargate)') {     // **one-line tweak**
      steps {
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-token']]) {
          script {
            def accountId = sh(script: 'aws sts get-caller-identity --query Account --output text', returnStdout: true).trim()
...
'''
    ...
    /*--------------------------------------------------------*/
    stage('Show service URL') {            // loop 60 iterations
      steps {
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-token']]) {
          sh '''#!/usr/bin/env bash
set -euo pipefail
